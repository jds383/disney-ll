...
function llText(ll) {
  if (ll === "ill") return "Individual Lightning Lane";
...
function Rankings({ parkId, prefs, onRdConfirm }) {
  ...
  const renderOverall = () =>
    scored.slice(0, 6).map((r, i) => {
      ...
      const rdTag = prefs[r.id]?.rdNom ? <span className="r-pill rd-tag">RD</span> : null;
      return (
        ...
        {rdNominees.length > 0 && (
          <div>
            ...
            {(rdConfirmed ? rdNominees.filter((r) => r.id === rdConfirmed) : rdNominees).map((r) => {
              ...
              return (
                <div className="r-item" key={r.id}>
                  <div className={`rd-chk${isConf ? " on" : ""}`} onClick={() => onRdConfirm(parkId, r.id)}>
                    {isConf ? "✓" : ""}
                  </div>
                  <span className="r-name">{r.displayName}</span>
...
export default function App() {
  ...
  const handleClosed = useCallback((rideId) => {
    setPrefs((prev) => {
      ...
      if (!cur) {
        next[rideId].rdNom = false;
        if (next[`rdc_${park}`] === rideId) next[`rdc_${park}`] = null;
      }
      saveStorage(next);
      return next;
    });
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ride-meta.meta-closed { color: #B71C1C; font-weight: bold; letter-spacing: 0.05em; }
      ...
      <div className="app">
        ...
        <Rankings parkId={activePark} prefs={prefs} onRdConfirm={handleRdConfirm} />
      </div>
    </>
  );
}
